...
class Restekonomi extends REST_Controller {

    function __construct($config = 'rest') {
        parent::__construct($config);
    }

    // show data ekonomi_id
    function index_get() {
        $id_ekonomi = $this->get('id_ekonomi');
        if ($id_ekonomi == '') {
            $ekonomi = $this->db->get('ekonomi')->result();
        } else {
            $this->db->where('id_ekonomi', $id_ekonomi);
            $ekonomi = $this->db->get('ekonomi')->result();
        }
        $this->response($ekonomi, 200);
    }
