...
class Restpanduan extends REST_Controller {

    function __construct($config = 'rest') {
        parent::__construct($config);
    }

    // show data panduan_id
    function index_get() {
        $id_panduan = $this->get('id_panduan');
        if ($id_panduan == '') {
            $panduan = $this->db->get('panduan')->result();
        } else {
            $this->db->where('id_panduan', $id_panduan);
            $panduan = $this->db->get('panduan')->result();
        }
        $this->response($panduan, 200);
    }
